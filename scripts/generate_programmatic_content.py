#!/usr/bin/env python3
"""Generate programmatic page/tool manifest from a single spreadsheet."""
import argparse
import os
import sys
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Generate all programmatic pages/tools from one spreadsheet",
    )
    parser.add_argument(
        "--source",
        default=os.environ.get("PROGRAMMATIC_CONTENT_SOURCE", DEFAULT_SOURCE_PATH),
        help="Path to spreadsheet (.csv/.tsv/.xlsx)",
    )
    parser.add_argument(
        "--sheet",
    ...
    if errors:
        print("Programmatic content generation failed:", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 1

    counts = manifest.get("counts", {})
    print("Programmatic content generated successfully")
    print(f"- source: {args.source}")
    print(f"- output: {args.output}")
    print(f"- total entries: {counts.get('total', 0)}")
